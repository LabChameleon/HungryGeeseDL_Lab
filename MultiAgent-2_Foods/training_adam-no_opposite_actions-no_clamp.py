...
from select_action import select_action_with_eps_non_opposite
...
def optimize_model():
    ...
    top2 = policy_net(non_final_next_states).topk(2)[1].detach()
    next_state_values[non_final_mask] = top2.gather(1, ((top2[:, 0] + 2) % 4 == action_batch[non_final_mask].squeeze()).long().unsqueeze(1)).squeeze().float()
...
with wandb.init(project='SingleAgent_OppositeDirection', name=run_name) as run:
    ...
    for i_episode in range(num_episodes):
        ...
        for cur_steps in count():
            ...
            steps_done += 1
            action = select_action_with_eps_non_opposite(game_map, policy_net, eps_threshold, n_actions,
                                                               last_action)
            last_action = action
