...
with wandb.init(project='DQN-MultiAgent-2_Food', name=run_name) as run:
    ...
    for i_episode in range(num_episodes):
        ...
        obs_list = [obs]
        game_map = create_map.create_map_from_obs_mulit_agent_individual_layer(obs_list, columns)
        ...
        for cur_steps in count():
            ...
            steps_done += 1
            action = select_action.select_action_with_eps_non_opposite(game_map, policy_net, eps_threshold, n_actions, last_action)
            last_action = action
            ...
            custom_reward = rf.snake_reward_function(obs_list, done or env_done, columns, cur_steps)

            if done or env_done:
                next_game_map = None
            else:
                next_game_map = create_map.create_map_from_obs_mulit_agent_individual_layer(obs_list, columns)
