...
policy_net = agent_resnet.DQN().to(device)
target_net = agent_resnet.DQN().to(device)
...
with wandb.init(project='DQN-MultiAgent-2_Food', name=run_name) as run:
    ...
    for i_episode in range(num_episodes):
        ...
        for cur_steps in count():
            ...
            steps_done += 1
            action = select_action.select_action_with_eps_non_opposite(game_map, policy_net, eps_threshold, last_action, n_actions)
            last_action = action
